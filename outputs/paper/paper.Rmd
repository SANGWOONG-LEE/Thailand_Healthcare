...
# create new dataset only for for education
 ...
 #                    )
# education <- data.frame(education = c(rep("no_education", data[8,6]), rep("primary", data[9,6]), rep("secondary", data[10,6]), rep("higher", data[11,6])),
#                     immune_data =  c(rep("Health Report", round(data[8,2]*data[8,6]/100)), rep("Mothers Report", round(data[8,3]*data[8,6]/100)), rep("No immune", round(data[8,5]*data[8,6]/100)), 
...
urban_rural %>%
  ggplot(aes(x = location, fill = immune_data)) +
...
region %>%
  ggplot(aes(x = region, fill = immune_data)) +
  geom_bar(position = "dodge")
```
```{r}
...
religion %>%
  ggplot(aes(x = religion, fill = immune_data)) +
  geom_bar(position = "dodge")
```
